...
/**
 * The PWIDController class implements a Proportional-Integral-Derivative
 * controller
...
public class PWIDController implements MotionController {
    ...
    double lastError = 0;
    // The accumulated error over time, which decays according to the integral
    // half-life.
     ...
     *
     * @param constants The PID constants including the decay rate for the integral
     *                  term.
     */
     ...
     *
     * @param currentError The current distance to the target from the current value
     ...
     * This should be called when the control target is changed or when the

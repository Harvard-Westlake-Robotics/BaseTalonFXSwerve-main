...
public class Pair<T> {
    ...
    /**
     * Applies a mapping function to both elements of the pair and returns a new
     * pair with the results.
     * 
     * @param <To>   The type of the elements in the resulting pair.
     * @param mapper A function that takes an element of type T and returns an
     *               element of type To.
     * @return A new Pair with each original element having been mapped to a new
     *         value.
     */
    public <To> Pair<To> map(Mapper<T, To> mapper) {
        // Maps both elements of the pair using the provided mapping function and
        // returns a new pair.
